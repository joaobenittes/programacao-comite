...
def calcular_boletim(aluno):
    ...
    aluno.update({'media': media, 'status_aluno': status_aluno})

    gerar_boletim(aluno)

def obtem_info():
    aluno = {'nome' : '', 'nota_1': 0, 'nota_2': 0, 'nota_3': 0}


    aluno['nome'] = input(f'informe o nome do aluno -> ')
    aluno['nota_1'] = float(input('informe a primeira nota -> '))
    aluno['nota_2'] = float(input('informea segunda nota -> '))
